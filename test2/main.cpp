...
void my_perror(const char* msg, int line)
{
        char buff[1024];
        snprintf(buff, sizeof(buff), "%s %d", msg, line);
        perror(buff);
}
...
int do_evp_seal(FILE *rsa_pkey_file, FILE *in_file, FILE *out_file)
{
        ...
        {
                my_perror("output file ", __LINE__);
                retval = 5;
                goto out_free;
        }
        ...
        while ((len = fread(buffer, 1, sizeof buffer, in_file)) > 0)
        {
                ...
                if (len_out > 0)
                {
                        int ret = fwrite(buffer_out, len_out, 1, out_file);
                        if (ret != 1)
                        {
                                printf("ret is: %d\n", ret);
                                my_perror("output file", __LINE__);
